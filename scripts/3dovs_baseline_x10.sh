...
do
    ...
    do
        ...
        do
            CUDA_VISIBLE_DEVICES=$((level+2)) nohup python train.py \
                ...
                > logs/langsplat/train/${casename}_train_lv${level}.log 2>&1 &
        done
        ...
        do
            CUDA_VISIBLE_DEVICES=$((level+2)) nohup python render.py \
                ...
                --include_feature \
                > logs/langsplat/render/${casename}_render_lv${level}.log 2>&1 &
        done
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES=$eval_gpu bash -c "
            cd eval
            nohup sh eval_3dovs.sh ${casename} > ../logs/langsplat/eval/${casename}_base${baseline_idx}_eval.log 2>&1
        " &
        ((eval_job_idx++))
    done
